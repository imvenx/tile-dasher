...
@onready var collected_gems: Array = loadItem('collected_gems', default_collected_gems)
func set_collected_gems(level: int, gem: String):
	collected_gems[level - 1].push_front(gem)
	saveItem('collected_gems', collected_gems)


const defaultUnlockedLevel = 1
@onready var last_unlocked_level = loadItem('last_unlocked_level', defaultUnlockedLevel)
func set_last_unlocked_level(val: int):
	last_unlocked_level = val
	saveItem('last_unlocked_level', val)
...
func loadItem(key: String, default_value = null):
	if FileAccess.file_exists(path):
		var file = FileAccess.open(path, FileAccess.READ)
		var loaded_data = JSON.parse_string(file.get_as_text()) as Dictionary
		file.close()
		var val = loaded_data.get(key, default_value)
...
func deleteAllStorage():
	...
	saveItem("collected_gems", default_collected_gems)
	saveItem("last_unlocked_level", defaultUnlockedLevel)
